# Source Rmd: ERGM/ERGM_Analysis/ERGM_Result1.Rmd
...
obs_stat <- sum(get.edge.attribute(net_wire, "w_scale"))

sim_stats <- sapply(sim_networks, function(net) {
  sum(get.edge.attribute(net, "w_scale"))
})
...
real_weights <- as.edgelist(net_wire, attrname = "w_scale")[, 3]
sim_weights <- unlist(lapply(sim_networks, function(net) as.edgelist(net, attrname = "w_scale")[, 3]))
...
## -----------------------------------------------------------------------------------------------------------------------------------------
# Convert to matrix
real_mat <- as.matrix(net_wire, attrname = "w_scale")
sim_mat <- as.matrix(sim_networks[[1]], attrname = "w_scale")
...
real_strength <- sna::degree(as.matrix(net_wire), gmode = "digraph", ignore.eval = FALSE)
...
hist(network::get.edge.attribute(net_wire, "w_scale"), breaks = 30,
     main = "Histogram of Edge Weights",
     xlab = "Edge Weight",
     col = "skyblue", border = "white")

# Add a vertical line for the mean weight
abline(v = mean(network::get.edge.attribute(net_wire, "w_scale")), col = "red", lwd = 2)
legend("topright", legend = paste("Mean =", round(mean(network::get.edge.attribute(net_wire, "w_scale")), 0)),
       col = "red", lwd = 2)
...
compute_edge_stats <- function(net) {
  ...
  ew <- network::get.edge.attribute(net, "w_scale")
...
obs_stats <- compute_edge_stats(net_wire)
...
compute_node_stats_efficient <- function(net) {
  ...
  weights <- network::get.edge.attribute(net, "w_scale")  # Extracts weight attribute
...
obs_node_stats <- compute_node_stats_efficient(net_wire)
...
obs_values <- c(
  ...
  obs_node_stats$summary$mean_binary_out_degreeSS
)
